...
- EN: Open the live playground on GitHub Pages: [https://cyberikun.github.io/aviator-expression-builder/](https://cyberikun.github.io/aviator-expression-builder/)
- 中文：可直接访问 GitHub Pages 在线体验： [https://cyberikun.github.io/aviator-expression-builder/](https://cyberikun.github.io/aviator-expression-builder/)

![Aviator Expression Builder Demo Preview](./assets/demo-preview.png)
...
const runtimeContext = {
  availableFields,
  parameterDataTypes: new Map<string, Map<string, string>>(),
  getFieldTypeForNode: (fieldPath: string) =>
    availableFields.find((field) => field.value === fieldPath)?.type ?? "string",
};

// Option A: one-shot helpers
const parsed = parseExpression("employee_id == 1", runtimeContext);
const errors = validateExpression(parsed.nodes);
const regenerated = generateExpression(parsed.nodes, runtimeContext);
...
- EN: For one-shot helper APIs (`parseExpression` / `generateExpression`), pass a full context (`availableFields`, `parameterDataTypes`, `getFieldTypeForNode`) to avoid parser runtime errors.
- 中文：使用 one-shot API（`parseExpression` / `generateExpression`）时，建议传完整 context（`availableFields`、`parameterDataTypes`、`getFieldTypeForNode`），避免解析期运行时错误。
...
### 5) Why does `parseExpression` throw `getFieldTypeForNode` errors? / 为什么 `parseExpression` 会报 `getFieldTypeForNode` 错误？
- EN: This usually means the helper API was called without a complete context. Pass `availableFields`, `parameterDataTypes`, and `getFieldTypeForNode`, or use `createAviatorExpressionService({ availableFields })`.
- 中文：通常是因为调用 helper API 时未提供完整 context。请传入 `availableFields`、`parameterDataTypes`、`getFieldTypeForNode`，或改用 `createAviatorExpressionService({ availableFields })`。
